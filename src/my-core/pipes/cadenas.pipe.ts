...
export class StripTagsPipe implements PipeTransform {
  ...
  transform(value: string, ...allowedTags: any[]): string {
    return allowedTags.length > 0
      ? value.replace(new RegExp(`<(?!\/?(${allowedTags.join('|')})\s*\/?)[^>]+>`, 'g'), '')
      : value.replace(/<(?:.|\s)*?>/g, '');
  }
}
...
@Pipe({name: 'url2seo'})
export class URL2SEOPipe implements PipeTransform {
  ...
  transform(value: string): any {
    ...
    return value.replace(/[^\u0000-\u007e]/g, (c) => diacriticsMap[c] || c);
  }
}

export const PIPES_CADENAS = [ElipsisPipe, CapitalizePipe, StripTagsPipe, URL2SEOPipe];
